...
GITHUB_REPO = os.getenv("REPO_URL", "http:\/\/github.com/IBM/data-prep-lab")
...
def update_markdown_content_updated(markdown: str, replacements: List[Tuple[str, str, str]]) -> str:
    ...
    if len(replacements) > 0:
        for target, old_value, new_value in replacements:
            log.info(f"Updating Link: text: [{target}], link: {old_value}, updated link: {new_value}")
            markdown = markdown.replace(old_value, new_value)
            # TODO: We should replace links and not paths other than links
            # markdown = markdown.replace(f"[{target}]({old_value})", f"[{target}]({new_value})")
    return markdown
