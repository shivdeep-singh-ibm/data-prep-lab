...
def check_and_update_title(df: pd.DataFrame):
    https_str = "https://"
    ...
    if (len(df["title"][0]) > len_str) and (df["title"][0][:len_str] == https_str):
        replace_title = True
        df["orig_title"] = df["title"]
        df["title"] = df["title"].apply(lambda x: x.replace(https_str, new_prefix, 1))
